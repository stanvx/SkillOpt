#!/usr/bin/env python3
"""SkillOpt-Sleep for Hermes Agent — minimal MCP server (stdio, stdlib-only).

Exposes the sleep engine as MCP tools, wired to harvest Hermes Agent sessions
(``~/.hermes/state.db``) and to run cycles through the Hermes CLI backend. No
third-party deps: speaks JSON-RPC 2.0 over stdio.

Tools mirror the shared engine (status / dry-run / run / adopt / harvest /
schedule / unschedule). Each shells out to `python -m skillopt_sleep <action>`
with backend and source defaulting to `hermes`. Configure your client to launch:
  python plugins/hermes/mcp_server.py
"""
...
TOOLS = [
    ...
    {"name": "sleep_adopt", "action": "adopt",
     "description": "Apply the latest staged proposal to the target skill/memory doc (backs up first)."},
    {"name": "sleep_harvest", "action": "harvest",
     "description": "Debug: list the recurring tasks mined from recent Hermes sessions."},
...
_TOOL_SCHEMA = {
    ...
    "properties": {
        ...
        "backend": {"type": "string", "enum": ["mock", "claude", "codex", "copilot", "hermes"],
                     "description": "mock = no API spend; hermes = drive the Hermes CLI (default)."},
        "scope": {"type": "string", "enum": ["invoked", "all"],
                  "description": "Harvest scope (default: invoked project only)."},
        "source": {"type": "string", "enum": ["claude", "codex", "hermes", "auto"],
                   "description": "Transcript source (default: hermes)."},
        "model": {"type": "string",
...
# Actions that harvest/replay and therefore want the Hermes backend + source by default.
...
def handle(req: dict):
    ...
    if method == "initialize":
        return _result(id_, {
            ...
            "capabilities": {"tools": {}},
            "serverInfo": {"name": "skillopt-sleep-hermes", "version": "0.1.0"},
        })
